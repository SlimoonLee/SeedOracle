...
状态：PR #43 已合并到作者 `main`；当前 API v5 主线提交为 `0552b33`。下文保留提交前的提案文本和验证记录，版本号演进仅作为历史证据。
...
这个 PR 提议增加一个公开、显式隔离的战前预测边界，让伴生 Mod 能复用 Combat Solver 的现有模拟与搜索实现，同时不修改玩家当前跑局。
...
提交前的审阅分支同时带有 worker 复用、资源状态、可配置保活和纯模拟能力；这些能力已随 PR #43 合并到作者主线。历史提案原计划按维护偏好拆成：
